...
- If hardware URO is enabled, and a socket opts-in to URO with a max coalesced size the same as, or larger than, the hardare offload, then TCP will deliver the NBLs from hardware unmodified.
...
The coalesced IP length field and the UDP length field must be updated with the coalesced length. The coalesced IPv4 checksum field must be updated with the new length. The coalesced UDP checksum field is ignored and does not need to be updated (since it was already validated individually).
